...
AtDCore.prototype.markMyWords = function() {
    ...
    for (var suggestionIndex = this.suggestions.length-1; suggestionIndex >= 0; suggestionIndex--) {
        ...
        if (!suggestion.used) {
            ...
            var cssName;
			if (ruleId == "ABKUERZUNG" || ruleId == "LANGES_WORT" || ruleId == "SATZ" || ruleId == "VERNEINUNG" || ruleId == "METAPHERN") {
				cssName = "bigMisstake";
			} else if (ruleId == "PRAETERITUM" || ruleId == "SONDERZEICHEN" || ruleId == "KONJUNKTIV" || ruleId == "PASSIV" || ruleId == "GENITIV" || ruleId == "INDIREKTE_REDE") {
				cssName = "middleMisstake";
			} else if (ruleId == "DATUM" || ruleId == "ZAHLWOERTER" || ruleId == "ZAHLEN" || ruleId == "ROEMISCHE_ZAHL" || ruleId == "FUSSNOTE") {
				cssName = "smallMisstake";
            } else if (ruleId.indexOf("SPELLER_RULE") >= 0 || ruleId.indexOf("MORFOLOGIK_RULE") == 0 || ruleId == "HUNSPELL_NO_SUGGEST_RULE" || ruleId == "HUNSPELL_RULE") {
...
AtDCore.prototype.isMarkedNode = function(node) {
    return (this.hasClass(node, 'hiddenGrammarError') || this.hasClass(node, 'hiddenSpellError') || this.hasClass(node, 'hiddenSuggestion') || this.hasClass(node, 'bigMisstake') || this.hasClass(node, 'middleMisstake') || this.hasClass(node, 'smallMisstake'));
};
